...
function downvoteComment(url, request){
  ...
  let savedComment = database.comments[id.toString()];
  const response = {};
  if(savedComment && database.comments[id] && database.users[username] !== undefined){
...
function deleteComment(url) {
  ...
  if(savedComment) {
    ...
    for(var i = 0; i < database.users[user].commentIds.length; i++){
      if(database.users[user].commentIds[i] === id){
        database.users[user].commentIds.splice(i, 1);
      }
    }
    for(var article in database.articles){
      // console.log(article === articleId.toString());
      if(article === articleId.toString()){
        for(var i = 0; i < database.articles[article].commentIds.length; i++){
          if(database.articles[article].commentIds[i] === id){
            database.articles[article].commentIds.splice(i, 1);
          }
        }
      }
    }
...
function updateComment(url, request){
  ...
  if(!savedComment){
    response.status = 404;
  } else if (!id || !requestComment || requestComment.body.length === 0) {
    response.status = 400;
  } else {
...
function createComment(url, request) {
  ...
  if(requestComment && (requestComment.body) && (database.users[requestComment.username]) && (database.articles[requestComment.articleId])){
    ...
    database.users[user].commentIds.push(comment.articleId);
    database.articles[articleId].commentIds.push(comment.id);
